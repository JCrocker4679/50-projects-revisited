...
- **Dark mode tests** (#69): `src/test/theme.test.ts` added (12 tests covering `getTheme`, `applyTheme`, `toggleTheme`, `computeInitialTheme`, and side-effect module load); `computeInitialTheme` pure helper extracted from `theme-init.ts` for testability; `updateThemeToggle` tests added to `ui.test.ts`; `theme.ts` and `theme-init.ts` both at 100% coverage (all metrics)
- **Playwright E2E setup**: `@playwright/test` installed; `playwright.config.ts` with Chromium only, `webServer` auto-starts Vite dev server, 1 retry in CI; `e2e/smoke.test.ts` placeholder confirms page loads and title/CTA visible; `test:e2e` script in `package.json`; Vitest `exclude` config updated to ignore `e2e/` directory
...
- **Search: data layer** (#61): `searchJokes(term, limit?)` added to `api.ts` — same AbortController/error-handling pattern as `fetchJoke()`; empty/whitespace terms rejected with `ApiError('validation')`; `SearchApiResponse` interface added to `types.ts`; `SearchState` fields (`isSearchActive`, `searchTerm`, `searchResults`, `selectedSearchIndex`) added to `AppState`; `setSearchResults`, `selectSearchResult`, `clearSearch`, `getSearchState`, `isSearchActive` added to `state.ts`; `navigateHistory()` returns null while search is active; MSW handler added for `/search` endpoint; 27 new tests (195 total)
- **Search: UI** (#62): Search form added to `index.html` (between heading and joke, `role="search"`, visually-hidden `<label>` for screen readers); search input + Search button + clear (✕) button; `<p id="searchMeta" aria-live="polite">` for accessible result count announcements; `<ul id="searchResults">` with `<li><button>` items (keyboard accessible); four new functions in `ui.ts`: `setSearchLoading`, `renderSearchResults`, `clearSearchUI`, `setHistoryNavDisabled`; `main.ts` wired up with `handleSearch`, `handleSelectSearchResult`, `handleClearSearch`; clicking a result collapses the list but stays in search mode (X button visible); "Tell me another" exits search mode and restores the prior joke; history nav disabled during search; all DOM construction uses `textContent` (XSS safe); min-height 44px on all interactive elements; `prefers-reduced-motion` respected; 30+ new tests
